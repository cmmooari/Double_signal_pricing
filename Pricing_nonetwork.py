...
class DA_OBD(object):
    ...
    def create_model(self, Param_dict):
        ...
        # model.weight = Param(initialize=0.53)
        model.subsidy = Param(initialize=0.2)
        ...
        model.w_SOC_max = Var(model.Time, model.Bnode, domain=Binary)
        model.w_SOC_min = Var(model.Time, model.Bnode, domain=Binary)
        model.w_R_max = Var(model.Time, model.Bnode, domain=Binary)
        model.w_R_min = Var(model.Time, model.Bnode, domain=Binary)
        model.w_Pin_min = Var(model.Time, model.Bnode, domain=Binary)
        model.w_Pout_min = Var(model.Time, model.Bnode, domain=Binary)
        ...
        def power_balance(model, t):
            return sum(model.P_L[t, k] for k in model.NonSnode) + sum(model.Pin[t, i] for i in model.Bnode) == \
                   sum(model.P0[t, j] for j in model.Snode) + sum(model.Pout[t, i] for i in model.Bnode)
        ...
        def obj_expression(model):
            return sum(sum(model.WP_p[t] * model.P0[t, j] for j in model.Snode) for t in model.Time) + \
        ...
        def obj_expression_weighted(model):
            return model.weight * sum(model.WP_p[t] * (sum(model.P_L[t, j] for j in model.NonSnode)
                                                       - sum(model.P0[t, k] for k in model.Snode)) for t in model.Time) \
                   + (1 - 2 * model.weight) * sum( - model.lambda_SOC[0, i] * model.SOCinit[i] + \
                       sum(model.mu_SOC_max[t, i] * model.SOCmax[i] for t in model.Time) - \
                       model.mu_SOC_min[23, i] * model.SOCend[i] - \
                       sum(model.mu_SOC_min[t, i] * model.SOCmin[i] for t in model.TimeX) + \
                       sum(model.mu_R_max[t, i] * model.Rmax[i] for t in model.Time) - \
                       ...
                       sum(model.Dlifetime[i] * model.R[t, i] for t in model.Time)
                       for i in model.Bnode) \
                   - (1 - model.weight) * sum(sum(model.Dlifetime[i] * model.R[t, i] for t in model.Time)
                       for i in model.Bnode)

        model.OBJ = Objective(rule=obj_expression)
        # model.OBJ = Objective(rule=obj_expression_weighted, sense=maximize)
    ...
    def get_coefficients(self, Vhat):
        ...
        for t in range(24):
            ...
            b = np.matmul(np.diag(self.V0.T[0]), (np.matmul(np.conjugate(self.Y00), np.conjugate(self.V0))
                ...
                + np.matmul(B, self.PL[t:t+1, :].T) + np.multiply(np.matmul(B, self.QL[t:t+1, :].T), np.array([1j]))
    ...
    def store_results(self):
        ...
        lambda_SOC = np.zeros((24, 89))
        mu_SOC_max = np.zeros((24, 89))
        mu_SOC_min = np.zeros((24, 89))
        mu_R_max = np.zeros((24, 89))
        mu_R_min = np.zeros((24, 89))
        mu_Pin_min = np.zeros((24, 89))
        mu_Pout_min = np.zeros((24, 89))
        w_SOC_max = np.zeros((24, 89))
        w_SOC_min = np.zeros((24, 89))
        w_R_max = np.zeros((24, 89))
        w_R_min = np.zeros((24, 89))
        w_Pin_min = np.zeros((24, 89))
        w_Pout_min = np.zeros((24, 89))
        ...
        for t in self.model.Time:
            for i in self.model.Bnode:
                ...
                lambda_SOC[t, i] = value(self.model.lambda_SOC[t, i])
                mu_SOC_min[t, i] = value(self. model.mu_SOC_min[t, i])
                mu_SOC_max[t, i] = value(self.model.mu_SOC_max[t, i])
                mu_R_min[t, i] = value(self.model.mu_R_min[t, i])
                mu_R_max[t, i] = value(self.model.mu_R_max[t, i])
                mu_Pin_min[t, i] = value(self.model.mu_Pin_min[t, i])
                mu_Pout_min[t, i] = value(self.model.mu_Pout_min[t, i])
                w_SOC_max[t, i] = value(self.model.w_SOC_max[t, i])
                w_SOC_min[t, i] = value(self.model.w_SOC_min[t, i])
                w_R_max[t, i] = value(self.model.w_R_max[t, i])
                w_R_min[t, i] = value(self.model.w_R_min[t, i])
                w_Pin_min[t, i] = value(self.model.w_Pin_min[t, i])
                w_Pout_min[t, i] = value(self.model.w_Pout_min[t, i])
        ...
        for t in self.model.Time:
            RP_E.append(value(self.model.subsidy) * WP_p[t] + (1-value(self.model.subsidy)) * (RP_B[t] * sum([Pout[t, i] - Pin[t, i] for i in self.model.Bnode]) + \
        ...
        np.savetxt('Results/lambda_SOC.csv', lambda_SOC, delimiter=',')
        np.savetxt('Results/mu_SOC_max.csv', mu_SOC_max, delimiter=',')
        np.savetxt('Results/mu_SOC_min.csv', mu_SOC_min, delimiter=',')
        np.savetxt('Results/mu_R_max.csv', mu_R_max, delimiter=',')
        np.savetxt('Results/mu_R_min.csv', mu_R_min, delimiter=',')
        np.savetxt('Results/mu_Pin_min.csv', mu_Pin_min, delimiter=',')
        np.savetxt('Results/mu_Pout_min.csv', mu_Pout_min, delimiter=',')
        np.savetxt('Results/w_SOC_max.csv', w_SOC_max, delimiter=',')
        np.savetxt('Results/w_SOC_min.csv', w_SOC_min, delimiter=',')
        np.savetxt('Results/w_R_max.csv', w_R_max, delimiter=',')
        np.savetxt('Results/w_R_min.csv', w_R_min, delimiter=',')
        np.savetxt('Results/w_Pin_min.csv', w_Pin_min, delimiter=',')
        np.savetxt('Results/w_Pout_min.csv', w_Pout_min, delimiter=',')


        # Plotting results
        datas = pd.DataFrame({'Wholesale Price': WP_p, 'Retail Battery Service Price': RP_B,
                              'Retail Energy Price': RP_E, 'System Load': [ x/20000 for x in np.sum(PL, axis=1).tolist()],
        ...
        print("Overall payment", sum(RP_E[t] * sum( value(self.model.P_L[t, i]) for i in self.model.NonSnode)
                                  for t in self.model.Time))
        ...
        print("Discounted revenue", sum(RP_B[t] * sum( value(self.model.Pout[t, i]) - value(self.model.Pin[t, i]) for i in self.model.Bnode) for t in self.model.Time)
              - sum( value(self.model.Dlifetime[i]) * sum(value(self.model.R[t, i]) for t in self.model.Time) for i in self.model.Bnode))
        ...
        q = sns.lineplot(x='Hour', y='Retail Battery Service Price', ax=ax2, data=datas, label=r'Retail grid service price', color='navy', marker='^')
...
if __name__ == "__main__":
    ...
    IEEE123.non_successive_solve(10)
    end = time.time()
