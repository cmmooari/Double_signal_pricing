...
class DA_OBD(object):
    ...
    def create_model(self, Param_dict):
        ...
        model.M = Param(initialize=50000)
        ...
        model.RP_e = Var(model.Time, domain=Reals)
        ...
        def price_equality_GS(model):
            return sum(model.RP_p[t] for t in model.Time) == 1.0

        def price_equality_E(model):
            return sum(model.RP_e[t] for t in model.Time) == 1.0

        def price_inequality_E(model, t):
            return 0.06 >= model.RP_e[t] >= 0

        model.powerbalance = Constraint(model.Time, rule=power_balance)
        model.priceEquP = Constraint(rule=price_equality_GS)
        model.priceEquE = Constraint(rule=price_equality_E)
        model.priceIneE = Constraint(model.Time, rule=price_inequality_E)

        # ------------------------------------ Objective function -------------------------------------
        def obj_expression(model):
            return sum(sum(model.WP_p[t] * model.P0[t, i] for i in model.Snode) for t in model.Time) + \
                   ...
                   - sum(sum(model.RP_e[t] * model.P_L[t, i] for i in model.NonSnode) for t in model.Time)
    ...
    def store_results(self):
        ...
        for t in self.model.Time:
            ...
            RP_E.append(value(self.model.RP_e[t]))
        ...
        # for t in self.model.Time:
            # RP_E.append((RP_B[t] * sum([Pout[t, i] - Pin[t, i] for i in self.model.Bnode]) + \
...
if __name__ == "__main__":
    ...
    dict_DA_OBD = {
        ...
        # 'list_of_Bnode': [16, 17, 18, 45, 48, 52, 55, 58, 61, 62, 63, 64, 65, 68, 69, 70, 72, 74, 77],
        # 'list_of_Bnode': [16, 17, 18, 45, 48, 52, 55, 58, 61, 62, 63, 64, 65, 68, 69, 70, 72, 74],
        # 'list_of_Bnode': [16, 17, 18, 45, 48, 52, 55, 58, 61, 62, 63, 64, 65, 68, 69, 70, 72],
        # 'list_of_Bnode': [16, 17, 18, 45, 48, 52, 55, 58, 61, 62, 63, 64, 65, 68, 69, 70],
        ...
        # 'list_of_Bnode': [16, 17, 18, 45, 48, 52],
        # 'list_of_Bnode': [16, 17, 18, 45, 48],
        # 'list_of_Bnode': [16, 17, 18, 45],
        # 'list_of_Bnode': [16, 17, 18],
        # 'list_of_Bnode': [16, 17],
        # 'list_of_Bnode': [16]
    }
